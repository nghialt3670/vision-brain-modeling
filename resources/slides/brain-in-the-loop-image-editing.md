...
## Progress Report: Vector-Guided Latent Steering via Cognitive Feedback

<div class="hero-line"></div>

**Weekly Progress Report: From Direct Neural Mapping to Interactive Latent Steering**
...
<div class="task-grid">
  <div class="task-card blue">
    <h3>1. Directional Intent Proof</h3>
    <p>Train EEGNet with <b>InfoNCE contrastive loss</b> ($\tau = 0.07$) to decode unit direction $\mathbf{d}\_{eeg} \in \mathbb{S}^{511}$ and execute 3-tier statistical proof suite.</p>
  </div>
  <div class="task-card purple">
    <h3>2. Dynamic Magnitude Decoding</h3>
    <p>Predict edit intensity $\Vert\Delta\mathbf{z}\Vert\_2 = f(B)$ from EEG to adaptively control step size $\alpha(B)$ (distinguishing subtle edits from major visual overhauls).</p>
  </div>
</div>

Note:
Lộ trình thực nghiệm gồm 2 mục tiêu cốt lõi: 
1) **Giải mã Hướng Ý định (Direction):** Huấn luyện EEGNet với hàm mất mát tương phản InfoNCE ($\tau = 0.07$) để triệt tiêu hiện tượng suy biến vector và chạy bộ 3 bài test thống kê.
2) **Giải mã Cường độ / Độ lớn (Magnitude):** Hiện tại chúng ta đang cố định bước nhảy $\alpha = 0.1$. Trong tương lai, việc giải mã độ lớn $\|\Delta \mathbf{z}\|_2$ từ sóng não sẽ giúp hệ thống tự điều chỉnh độ nhích lớn hay nhỏ tùy thuộc vào ý định chỉnh sửa nhẹ hay thay đổi toàn bộ của người dùng.

Our empirical roadmap focuses on two core objectives:
1) **Directional Intent Decoding:** Train EEGNet with InfoNCE contrastive loss ($\tau = 0.07$) to eliminate vector collapse and run our 3-tier statistical proof suite for directional unit vectors.
2) **Dynamic Magnitude Decoding:** In addition to unit direction, predict edit intensity $\|\Delta\mathbf{z}\|_2 = f(B)$ from EEG signals to dynamically adapt step size $\alpha(B)$, differentiating subtle touch-ups from major visual transformations.
